...
google.devrel.samples.hello.getGreeting = function(id,zoominput,username) {
  gapi.client.helloworld.greetings.getGreeting({'group': id,'zoom': zoominput,'username':username}).execute(
      function(resp) {
        if (!resp.code) {
...
function fetchDatasetsList(){
 var datalist = httpGet('https://geojsonsimplifier.appspot.com/_ah/api/helloworld/v1/listDataSets/www/1/'+sessionStorage.userinfo);
